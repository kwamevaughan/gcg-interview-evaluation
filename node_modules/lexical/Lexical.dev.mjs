...
class RangeSelection {
  ...
  getNodes() {
    ...
    let firstNode = firstPointNode;
    let lastNode = lastPointNode;
    let overselectedFirstNode = false;
    const overselectedLastNodes = new Set();
    if ($isElementNode(firstPointNode)) {
      overselectedFirstNode = firstPoint.offset > 0 && firstPoint.offset >= firstPointNode.getChildrenSize();
      firstNode = firstPointNode.getDescendantByIndex(firstPoint.offset) || firstPointNode;
    }
    if ($isElementNode(lastPointNode)) {
      ...
      if (lastPointChild) {
        ...
        lastNode = $isElementNode(lastPointChild) && lastPointChild.getFirstDescendant() || lastPointChild;
        for (let overselected = lastNode; overselected && !overselected.is(lastPointChild); overselected = overselected.getParent()) {
          overselectedLastNodes.add(overselected.getKey());
        }
      } else {
        const beforeChild = lastPoint.offset > 0 && lastPointNode.getChildAtIndex(lastPoint.offset - 1);
        if (beforeChild) {
          // This case is not an overselection
          lastNode = $isElementNode(beforeChild) && beforeChild.getLastDescendant() || beforeChild;
        } else {
    ...
    if (firstNode.is(lastNode) || this.isCollapsed()) {
      ...
      if (overselectedLastNodes.size > 0) {
        while (nodes.length > 0 && overselectedLastNodes.has(nodes[nodes.length - 1].getKey())) {
          nodes.pop();
        }
      }
        ...
        for (const caret of initialRange.iterNodeCarets('shadowRoot')) {
          if ($isChildCaret(caret)) {
            if (caret.origin.isInline()) ; else if (caret.origin.isShadowRoot()) {
              ...
              $updateRangeSelectionFromCaretRange(this, $getCaretRange(!caret.origin.isEmpty() && state.block.isEmpty() ? $rewindSiblingCaret($getSiblingCaret(state.block, caret.direction)) : initialRange.anchor, caret));
...
LexicalEditor.version = "0.25.0+dev.esm";
...
function $getTextNodeOffset(origin, offset) {
  ...
  if (!(numericOffset >= 0 && numericOffset <= size)) {
    throw Error(`$getTextPointCaret: invalid offset ${String(offset)} for size ${String(size)}`);
  }
...
class CaretRangeImpl {
  ...
  iterNodeCarets(rootMode = 'root') {
    const anchor = $isTextPointCaret(this.anchor) ? this.anchor.getSiblingCaret() : this.anchor;
    const {
      focus
    } = this;
