...
;
async function upsertCandidate({ fullName, email, phone, linkedin }) {
    try {
        console.log("Checking for existing candidate with email:", email);
        const { data: existingCandidate, error: fetchError } = await supabase.from("candidates").select("id").eq("email", email).single();
        ...
        if (existingCandidate) {
            ...
            const { data: updatedCandidate, error: updateError } = await supabase.from("candidates").update({
                full_name: fullName,
                phone,
                linkedin
            ...
            const { data: newCandidate, error: insertError } = await supabase.from("candidates").insert([
                {
                    full_name: fullName,
                    email,
                    phone,
...
"[project]/src/data/questions.js [api] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
;
;
async function sendEmails({ fullName, email, phone, linkedin, score, resumeUrl, coverLetterUrl, answers, candidateTemplate, adminTemplate }) {
    try {
        ...
        const candidateHtml = candidateTemplate.replace("{{fullName}}", fullName).replace("{{score}}", score);
...
async function handler(req, res) {
    ...
    try {
        ...
        const score = (0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$scoreUtils$2e$js__$5b$api$5d$__$28$ecmascript$29$__["calculateScore"])(answers);
        // Fetch existing response to get old file IDs
        const { data: existingResponse, error: fetchResponseError } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$js__$5b$api$5d$__$28$ecmascript$29$__["supabase"].from("responses").select("resume_file_id, cover_letter_file_id").eq("user_id", userId).single();
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__37af4a._.js.map