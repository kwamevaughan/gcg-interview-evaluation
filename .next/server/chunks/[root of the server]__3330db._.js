...
async function handler(req, res) {
    ...
    try {
        ...
        console.log("Attempting to insert into candidates with data:", {
            full_name: fullName,
            email,
            phone,
            linkedin
        });
        const { data: userData, error: userError } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$js__$5b$api$5d$__$28$ecmascript$29$__["supabase"].from("candidates").insert([
            {
                full_name: fullName,
                email,
                phone,
        ...
        let auth;
        try {
            auth = new __TURBOPACK__imported__module__$5b$externals$5d2f$googleapis__$5b$external$5d$__$28$googleapis$2c$__cjs$29$__["google"].auth.GoogleAuth({
        ...
        if (resume) {
            try {
                ...
                const resumeExt = resume.startsWith("JVBERi0") ? "pdf" : "docx"; // Basic PDF check
                const resumeFileName = `${userId}-resume-${Date.now()}.${resumeExt}`;
                const resumeBuffer = Buffer.from(resume, "base64");
                console.log("Resume size (bytes):", resumeBuffer.length);
                const fileMetadata = {
                    name: resumeFileName,
                    parents: [
                ...
                const media = {
                    mimeType: resumeExt === "pdf" ? "application/pdf" : "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    body: Buffer.isBuffer(resumeBuffer) ? resumeBuffer : Buffer.from(resumeBuffer)
                };
                ...
                // Continue without the resume rather than failing the whole process
                resumeUrl = null;
            }
        }
        ...
        if (coverLetter) {
            try {
                console.log("Processing cover letter upload...");
                const coverLetterExt = coverLetter.startsWith("JVBERi0") ? "pdf" : "docx";
                const coverLetterFileName = `${userId}-cover-letter-${Date.now()}.${coverLetterExt}`;
                const coverLetterBuffer = Buffer.from(coverLetter, "base64");
                console.log("Cover letter size (bytes):", coverLetterBuffer.length);
                const fileMetadata = {
                ...
                const media = {
                    mimeType: coverLetterExt === "pdf" ? "application/pdf" : "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    body: Buffer.isBuffer(coverLetterBuffer) ? coverLetterBuffer : Buffer.from(coverLetterBuffer)
                };
                const driveResponse = await drive.files.create({
                    requestBody: fileMetadata,
                    media: media,
                ...
                coverLetterUrl = driveResponse.data.webViewLink;
                console.log("Cover letter uploaded successfully:", coverLetterUrl);
            } catch (uploadError) {
                console.error("Cover letter upload error:", uploadError);
                // Continue without the cover letter rather than failing the whole process
                coverLetterUrl = null;
            }
        }
        // Insert into responses table
        console.log("Attempting to insert into responses with data:", {
            user_id: userId,
            score,
            resume_url: resumeUrl,
            cover_letter_url: coverLetterUrl
        });
        const { error: responseError } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$js__$5b$api$5d$__$28$ecmascript$29$__["supabase"].from("responses").insert([
            {
                user_id: userId,
        ...
        // Continue the process even if emails fail
        }
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__3330db._.js.map